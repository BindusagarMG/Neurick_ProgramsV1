        ...
        else
        {
            ...
            else
            {
                if (fb->format == PIXFORMAT_RGB565
                    ...
                    s = fmt2jpg(fb->buf, fb->len, fb->width, fb->height, PIXFORMAT_RGB565, 90, &_jpg_buf, &_jpg_buf_len);
                    esp_camera_fb_return(fb);
                    fb = NULL;
                    if (!s) {
...
{
    #if CONFIG_LED_ILLUMINATOR_ENABLED
    ledcAttachChannel(pin, 5000, 8, LED_LEDC_CHANNEL);
    #else
